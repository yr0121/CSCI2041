...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | While (cond, body) ->
     (match eval cond stt with
      | Int _->raise (Failure "Type error on While")
      | Bool false -> stt
      | Bool true -> 
  ...
  | Skip->stt
  | For(nm,e1,e2,body)->
        (match eval (Eq(e1,Add(e2,Val(Int 1)))) stt with
        |Bool true->stt
        |Bool false->exec (Seq(body,For(nm,Add(e1,Val(Int 1)),e2,body))) (exec (Assign(nm,e1)) stt)
        |_->raise(Failure "Type error on For")
      )
      
      

let num_sum=11
let val_sum_evens=Int 0
let val_sum_odds=Int 0
let num_sum_evens=9
let num_sum_odds=8
